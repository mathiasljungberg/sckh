...
contains
  ...
  subroutine compute_SCXAS(E_i, E_f, E_fi_mean, D_fi, time, sigma_m, gamma)
    ...
    do k =1,nfinal! final state 
       do m=1,3 ! polarization    
          ...
          funct = D_fi(k,1,m) * D_fi(k,:,m) * e_factor1(k,:) * exp(-gamma * eV * time(:) / hbar) !*  exp(-gamma * (eV * time(:) / hbar)**2 ) 
  ...
  subroutine compute_SCXAS_new(corr_fn, time, sigma_m, gamma)
    ...
    do k =1,nfinal! final state 
       do m=1,3 ! polarization    
          ...
          funct = corr_fn(k,:,m) *exp(-gamma * eV * time(:) / hbar) !*  exp(-gamma * (eV * time(:) / hbar)**2 ) 
  ...
  subroutine compute_corr_fun_FT(E_i, E_f, E_fi_mean, D_fi, time, gamma, corr_fn_FT_tmp)
    ...
    do k =1,nfinal! final state 
       do m=1,3 ! polarization    
          ...
          funct = D_fi(k,:,m) * e_factor1(k,:) * exp(-gamma * eV * time(:) / hbar) !*  exp(-gamma * (eV * time(:) / hbar)**2 ) 
  ...
  subroutine convolute_gaussian(x,y, fwhm)
    ...
    do i=1, size(x)
       do j=1, size(x)
          y2(i) = y2(i) + y(j) * (alpha / pi)**0.5_wp * exp(-alpha*(x(i)-x(j))**2.0_wp)  
       end do
    end do
