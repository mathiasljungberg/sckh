...
contains
  ...
  subroutine calculate_SCXAS_PES(p)
    ...
    real(kind=wp),allocatable:: sigma_f_om_mm(:,:,:,:)
    ...
    real(kind=wp):: gamma, time_l, delta_t, norm, E_fi_mean, mu_SI, dx
    ...
    allocate( E_i_inp(npoints_in),&
         ...
         D_fi_inp(nfinal,npoints_in,3), &
         time(ntsteps),&
         !E_n(ntsteps), &
         E_f(nfinal,ntsteps), &
         D_fi(nfinal,ntsteps,3), &
    ...
    do j=1,p % npesfile_f
      ...
      call read_dipole_file(p % dipolefile_f(j), p % npoints_in, p % npoints_in, X_r, D_fi_inp(j,:,:))
    end do
    ...
    n_omega = ntsteps
    allocate(sigma_f_om_mm(nfinal,n_omega,3,3), &
         sigma_tmp(nfinal,n_omega,3,3), &
         sigma_f(nfinal,n_omega), &
         sigma_tot(n_omega), &
    ...
    do traj=1, npoints_x_mom_sampl
      ...
      do i=1,nfinal
        do m=1,3
          call spline_easy(X_r, D_fi_inp(i,:,m) , npoints_in, x_new, D_fi(i,:,m) , ntsteps)  
        end do
      end do
      ...
      if (traj .eq. 1) then
        ind = minloc(E_i_inp)
        E_fi_mean =  E_f(max(1,nfinal/2),ind(1)) - E_i(ind(1))
        write(6,*) "E_fi_mean", E_fi_mean

        call get_omega_reordered_fftw(time_l * const % eV /  const % hbar, omega)
        omega = omega + E_fi_mean
      end if

      call compute_SCXAS_sigma_f_om_mm(E_i, E_f, E_fi_mean, D_fi, time, sigma_tmp, gamma)
      
      sigma_f_om_mm = sigma_f_om_mm + sigma_tmp
    ...
    sigma_f(:,:) = 0.0_wp
    do m1=1,3
      sigma_f(:,:) = sigma_f(:,:) + sigma_f_om_mm(:,:,m1,m1)
    end do
      ...
      do m1=1,3
        do m2=1,3
          sigma_proj(i,:) = sigma_proj(i,:) + p % projvec(i,m1) * &
               sum( sigma_f_om_mm(:,:,m1,m2),1) * p % projvec(i,m2)  
        end do
      end do
    end do
