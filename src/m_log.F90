...
subroutine log_memory_note(mesg, iv_in, line, iter)
  ...
  if(present(line) .and. present(iter)) then
    write(string,'(a,i8,a,i8)') adjustr(trim(message))//&
      ' at ', line, ' iter ', iter
  else if(present(line) .and. (.not. present(iter))) then
    write(string,'(a,i8)') adjustr(trim(message))//&
      ' at ', line
  else 
  ...
  if(log_memory_note_first==1) then
    cmd = 'echo -n "'//trim(string)//' ">'//trim(fname)
  else
    cmd = 'echo -n "'//trim(string)//' ">>'//trim(fname)
  endif
  !if(iv>0)write(ilog,*) trim(cmd)
  call system(cmd)

  call getpid(pid)
  if(log_memory_note_first==1) then
    write(cmd,'(a,i8,a,a)')'ps -p ',pid, ' -o vsz,rsz,pmem >>',trim(fname)
    if(iv>0)write(ilog,'(a,a,a,a)') ' memory_note:', trim(cmd),' ',trim(string)
    call system(cmd)
  endif

  write(cmd,'(a,i8,a,a)')'ps h -p ',pid, ' -o vsz,rsz,pmem >>',trim(fname)
  if(iv>0)write(ilog,'(a,a,a,a)') ' memory_note:', trim(cmd),' ',trim(string)
  call system(cmd)
...
subroutine log_size_note_double(vname, size_MB, iv1, ctag)
  ...
  if(isizes>0) then
    if(present(ctag)) then
      write(isizes,'(a50,2x,g15.6,3x,a)') trim(vname), size_MB, trim(ctag);
    else
      write(isizes,'(a50,2x,g15.6)') trim(vname), size_MB;
    endif
      
    if(iv>0)write(ilog, '(a,a50,2x,g15.6)')' size_note: ', trim(vname), size_MB;
  else
    _die('init log file first')
  endif
...
subroutine log_size_note_double_array(vname, vals, iv1, ctag)
  ...
  n = ub - lb + 1
  if(n>0) then
    write(fstring,'(a,i10.0,a)')'(a50,2x,',size(vals),'g15.6,3x,a)'
    write(fstring_out,'(a,i10.0,a)')'(a,a50,2x,',size(vals),'g15.6,3x,a)'
  else 
...
subroutine log_size_note_complex8(vname, param, iv1, mynode1)
  ...
  if(isizes>0) then
          write(isizes,   '(a50,2x,g15.6,g15.6)') trim(vname), param;
    if(iv>0)write(ilog, '(a,a50,2x,g15.6,g15.6)')' size_note: ', trim(vname), param;
  else
    mynode=0; if(present(mynode1)) mynode=mynode1
