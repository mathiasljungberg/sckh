...
subroutine  input_calc_params(fname, inp, iv_in, ilog);
  ...
  if(ido>0) then
    ...
    do i=1,size(inp%lines); write(ifile,'(i5,a)') i, trim(inp%lines(i)); enddo
    close(ifile)
...
subroutine init_param_char(param_name, inp, def_value, actual_value, iv)
  ...
  if(len_trim(actual_value)<1) actual_value = def_value
...
subroutine get_param_char(param_name, line, res)
  ...
  if(ios/=0) then
    call warn('ios/=0 '//__FILE__, __LINE__)
    res=''
  endif  
...
subroutine init_param_int_array(param_name, inp, def_value, actual_value, iv, do_not_report)
  ...
  actual_value = def_value
  l = get_line_num(param_name, inp)
  if(l>0) call get_param_int_array(param_name, inp%lines(l), actual_value)
...
function get_line_num(param_name, inp) result(iline_found)
  ...
  do iline=1,nsize
    ...
    if(iline_found==-1) then
      ...
      write(6,'(a,2i6)') 'check lines: ', iline_found, iline
      write(6,'(a,i6,a)') 'line: ', iline_found, inp%lines(iline_found)
      write(6,'(a,i6,a)') 'line: ', iline, inp%lines(iline)
      iline_found = iline
      _die('appears twice');
    endif  
  enddo
