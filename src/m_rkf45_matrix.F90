...
subroutine rkfs_matrix_c (  neqn1, neqn2, f, y, t, tout, relerr, abserr, iflag, yp, h, &
  f1, f2, f3, f4, f5, savre, savae, nfe, kop, init, jflag, kflag )
...
subroutine rkfs_matrix_c_diagonal (  neqn1, f, y, t, tout, relerr, abserr, iflag, yp, h, &
  f1, f2, f3, f4, f5, savre, savae, nfe, kop, init, jflag, kflag )
...
!
  ...
  real(kind=wp):: et
  !external f
  complex(kind=wp):: f1(neqn1)
  complex(kind=wp):: f2(neqn1)
  complex(kind=wp):: f3(neqn1)
  complex(kind=wp):: f4(neqn1)
  complex(kind=wp):: f5(neqn1)
  real(kind=wp):: h
  logical hfaild
  real(kind=wp):: hmin
  integer iflag
  ...
  real(kind=wp):: toln
  real(kind=wp):: tout
  complex(kind=wp):: y(neqn1)
  complex(kind=wp):: yp(neqn1)
  real(kind=wp):: ypk
...
!
  if ( abs ( dt ) <= 26.0D+00 * eps * abs ( t ) ) then
    y(1:neqn1) = y(1:neqn1) + dt * yp(1:neqn1)
    a = tout
...
!  Advance an approximate solution over one step of length H.
!
  call fehl_matrix_diagonal ( f, neqn1, y, t, h, yp, f1, f2, f3, f4, f5, f1 )
  nfe = nfe + 5 
!
!  Compute and test allowable tolerances versus local error estimates
!  and remove scaling of tolerances.  Note that relative error is
...
!
  ...
  return
end subroutine rkfs_matrix_c_diagonal

subroutine fehl_matrix_diagonal ( f, neqn1, y, t, h, yp, f1, f2, f3, f4, f5, s )
...
!  implicit none
!
  integer, intent(in):: neqn1
!
  !external:: f
  complex(kind=wp), intent(out):: f1(neqn1)
  complex(kind=wp), intent(out)::  f2(neqn1)
  complex(kind=wp), intent(out)::  f3(neqn1)
  complex(kind=wp), intent(out)::  f4(neqn1)
  complex(kind=wp), intent(out)::  f5(neqn1)
  real(kind=wp), intent(in)::  h
  complex(kind=wp), intent(out)::  s(neqn1)
  real(kind=wp), intent(in):: t
  complex(kind=wp), intent(in):: y(neqn1)
  complex(kind=wp), intent(in):: yp(neqn1)
!
  real(kind=wp):: ch
  ...
  f5(1:neqn1) = y(1:neqn1) + ch * yp(1:neqn1)

  call f ( t + ch, f5, f1 )

  ch = 3.0D+00 * h / 32.0D+00

  f5(1:neqn1) = y(1:neqn1) + ch * ( yp(1:neqn1) + 3.0D+00 * f1(1:neqn1) )

  call f ( t + 3.0D+00 * h / 8.0D+00, f5, f2 )

  ch = h / 2197.0D+00

  f5(1:neqn1) = y(1:neqn1) + ch * ( 1932.0D+00 * yp(1:neqn1) &
    + ( 7296.0D+00 * f2(1:neqn1) - 7200.0D+00 * f1(1:neqn1) ) )

  call f ( t + 12.0D+00 * h / 13.0D+00, f5, f3 )

  ch = h / 4104.0D+00

  f5(1:neqn1) = y(1:neqn1) + ch * ( ( 8341.0D+00 * yp(1:neqn1) &
    - 845.0D+00 * f3(1:neqn1) ) + ( 29440.0D+00 * f2(1:neqn1) &
    - 32832.0D+00 * f1(1:neqn1) ) )

  call f ( t + h, f5, f4 )

  ch = h / 20520.0D+00

  f1(1:neqn1) = y(1:neqn1) + ch * ( ( -6080.0D+00 * yp(1:neqn1) &
    + ( 9295.0D+00 * f3(1:neqn1) - 5643.0D+00 * f4(1:neqn1) ) ) &
    + ( 41040.0D+00 * f1(1:neqn1) - 28352.0D+00 * f2(1:neqn1) ) )
...
!
  ...
  s(1:neqn1) = y(1:neqn1) + ch * ( ( 902880.0D+00 * yp(1:neqn1) &
    + ( 3855735.0D+00 * f3(1:neqn1) - 1371249.0D+00 * f4(1:neqn1) ) ) &
    + ( 3953664.0D+00 * f2(1:neqn1) + 277020.0D+00 * f5(1:neqn1) ) )

  return
end subroutine fehl_matrix_diagonal
