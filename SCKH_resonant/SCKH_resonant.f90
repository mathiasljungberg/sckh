...
!    end do
  ...
  subroutine verlet_trajectory_traj(traj1, x_start, v_start, x, E_i_inp, my_SI)
  ...
  end subroutine verlet_trajectory_traj


  subroutine compute_F_if_omp_t(trajectory2, E_nf_mean, fftw_plan, fftw_in, fftw_out)
    
    ! now compute F_{if, \omega'}(t) by FFTW
    do i=1,nfinal
      do m1=1,3
        do m2=1,3
          func_in = 0.0_wp
          do j=1,ninterm
            ...
            call compute_efactor(E_n_t2(j,:), E_f_t2(i,:), E_nf_mean, time2, efactor_nf, .false.)      
            
            ! simplest case, no dipoles
            func_omp_in = func_omp_in + efactor_nf
            
          end do
          
          func_in = func_in * exp(-gamma * (eV / hbar)* time2)
          
          call dfftw_execute_dft ( fftw_plan, fftw_in, fttw_out )
                    
          F_if_omp_t(f, m1,m2,:,traj2) = func_out ! normalisering? 1 / N ?
          
        end do ! m2=1,3
      end do ! do m1=1,3
    end do !do i=1,nfinal
    
  end subroutine compute_F_if_omp_t


    subroutine compute_F_if_omp_om(F_if_omp_t, E_nf_mean, fftw_plan, fftw_in, fftw_out, F_if_omp_om)

      !Fourier transform  F_{if, \omega'}(t)
      F_if_omp_om = 0.0_wp
      do fin =1, nfinal
        do m1=1,3
          do m2=1,3
            do omp =1, n_omega_p
              ...
              func_om_in = F_if_omp_t(fin, m1, m2, omp,:) * exp(dcmplx(0, (eV  / hbar) * ( omega_p(omp) - E_nf_mean ) * time)) *&
                   exp(-detector_FWHM * eV * time / (2.0_wp * hbar))
              ...
              ! add this to the total
              !F_f_omp_om_tot(fin, m1,m2,omp,:) = F_f_omp_om_tot(fin,m1,m2,omp,:) + F_f_omp_om(fin,m1,m2,omp,:) !F_f_om_t(fin, m1,m2,om,:) !F_f_om_om(fin,m1,m2,om,:)
              F_f_omp_om_tot(fin, m1,m2,omp,:) = F_f_omp_om_tot(fin,m1,m2,omp,:) + func_om_out
              

            ! the t=0 contribution
            !F_f_omp_0_tot(fin, m1,m2,omp) = F_f_omp_0_tot(fin,m1,m2,omp) + F_f_omp_t(fin,m1,m2,omp,1) !F_f_om_t(fin, m1,m2,om,:) !F_f_om_om(fin,m1,m2,om,:)
            !F_f_omp_0_tot(fin, m1,m2,omp) = F_f_omp_0_tot(fin,m1,m2,omp) + F_f_omp_t(fin,m1,m2,omp,1) 

          end do! do omp =1, n_omega_p
        end do ! do m2=1,3
      end do ! do m1=1,3
    end do ! do fin =1, nfinal
