...
contains
  ...
  function  sinc(x, x_n, dx) 
    ...
    ! normalized sinc function  =   sqrt(dx)*sin(pi * (x-x_n) / dx) / (pi * (x-x_n))
    ...
    if ( abs((x-x_n)/dx) .lt. 1.0d-13) then
       sinc = sqrt(dx)* ( 1.0d0/dx  - (pi**2/(6*dx**3))*(x-x_n)**2 )
       
       !write(7,*) (x-x_n), sinc, sin(pi * (x-x_n) / dx) / (pi * (x-x_n))
       
       !otherwise 
    else
       sinc = sqrt(dx)*sin(pi * (x-x_n) / dx) / (pi * (x-x_n))
    end if
  ...
  function  dsinc(x, x_n, dx) 
    ...
    ! derivative of normalized sinc function  = D/Dx  sqrt(dx)*sin(pi * (x-x_n) / dx) / (pi * (x-x_n))
    ! =  pi * ((1/sqrt(dx)) * cos(pi * (x-x_n) / dx)/ (pi * (x-x_n))  - sqrt(dx) * sin(pi * (x-x_n) / dx) / (pi * (x-x_n))**2 ) 
    
    ! case when x close to X_n  (expand in taylor series)
    if ( abs(x-x_n) .lt. 1.0d-13) then
       dsinc = sqrt(dx)* (  - (pi**2/(3*dx**3))*(x-x_n) )
       
       !otherwise 
    else
       dsinc = ( (1.0d0 / sqrt(dx)) *  cos(pi * (x-x_n) / dx) / ( (x-x_n)) - &
            sqrt(dx) * sin(pi * (x-x_n) / dx) / (pi * (x-x_n) **2))
    end if
  ...
  function  d2sinc(x, x_n, dx) 
    ...
    ! second derivative of normalized sinc function  = d/dx  sqrt(dx)*sin(pi * (x-x_n) / dx) / (pi * (x-x_n))
    ! =  pi * ((1/sqrt(dx)) * cos(pi * (x-x_n) / dx)/ (pi * (x-x_n))  - sqrt(dx) * sin(pi * (x-x_n) / dx) / (pi * (x-x_n))**2 ) 
    
    ! case when x close to X_n  (expand in taylor series)
    if ( abs(x-x_n) .lt. 1.0d-13) then
       d2sinc = sqrt(dx)* pi**2 * (  - (1.0d0 / (3*dx**3))+ &
            (1.0d0 / dx**5 ) * (pi* (x-x_n))  **2 )
       
       !otherwise 
    else
       d2sinc =  -sqrt(dx) * pi**2 * ( (1.0d0 / dx**2) *  sin(pi * (x-x_n) / dx) / ( pi*(x-x_n) ) + &
            (2.0d0 / dx) * cos(pi * (x-x_n) / dx) / ( (pi * (x-x_n)) **2) - &
            2.0d0 * sin(pi * (x-x_n) / dx) / ( (pi * (x-x_n)) **3) )
    end if
