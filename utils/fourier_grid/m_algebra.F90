...
#include "m_define_macro.F90"
  ...
  public create_left_eigenvectors_d
  ...
  public get_volume
  public dSymmGeneralEigen_lin_dep
  public compute_neg_root_lin_dep 
  public solve_gen_eig_lin_dep
...
! get volume of a parallelepipied
 function get_volume(uc_vecs) result(volume)
    !use m_algebra, only : cross_product

    real(8), intent(in):: uc_vecs(3,3) 
    real(8) :: volume

    volume = abs(dot_product( uc_vecs(:,1),cross_product(uc_vecs(:,2),uc_vecs(:,3))))


  end function get_volume

  
subroutine create_left_eigenvectors_d(X_R, X_L)
  real(8),intent(in):: X_R(:,:)
  real(8),intent(out):: X_L(:,:)
  
  real(8),allocatable:: S(:,:), S_inv(:,:)
  integer:: n
  
  ! assumes convention X_R(ao, mo)
  n= size(X_R,1)
  
  allocate(S(n,n))
  allocate(S_inv(n,n))
  
  call matmul_AtB_d(X_R, X_R, S)
  
  S_inv = S
  call matinv_d(S_inv)
  
  call matmul_AB_d(X_R, S_inv, X_L)
  
end subroutine create_left_eigenvectors_d


! 
subroutine dSymmGeneralEigen_lin_dep(H,S,E,X, tol_in)
  use m_precision, only : blas_int
  implicit none
  real(8), intent(in)  :: H(:,:), S(:,:)
  real(8), allocatable, intent(out) :: X(:,:), E(:)
  real(8), intent(in), optional:: tol_in

  real(8):: tol
  real(8), allocatable:: E_S(:), X_S(:,:), S_nrt(:,:), X_mm(:,:) ,&
       H_new(:,:)

  integer:: n,m, start, i,j
  ...
  ! first diagonalize S
  allocate(E_S(n), X_S(n, n))
  call diagonalize(S, E_S, X_S)

  ! remove linear dependencies
  start=0
  do i=1, n
    if(E_S(i) .gt. 0.0d0 .and. &
         maxval(abs(E_S))/abs(E_S(i)) .lt. tol  ) then
      start=i 
      exit
    else
      write(6,*) "removed eigenvalue ", E_S(i)
    end if
  end do

  if (start .eq. 0) then
    write(6,*) "problem in dSymmGeneralEigen_lin_dep: no suitable eigenvalues found"
    stop
  end if
  ...
  do j=1,m
    do i=1,n
      S_nrt(i,j) = X_S(i,j+start-1) * E_S(j+start-1)**(-0.5d0) 
    end do
  end do
  ...
  ! construct new H matrix
  call matmul_AtBC_d(S_nrt, H, S_nrt, H_new)

  ! diagonalize it
  call diagonalize(H_new, E, X_mm)

  call matmul_AB_d(S_nrt, X_mm, X)

  ! test (reuse X_mm as temp array)
  call matmul_AtBC_d(X, H, X, X_mm)

  write(6,*) "X' H X ", X_mm 

  call matmul_AtBC_d(X, S, X, X_mm)

  write(6,*) "X' S X ", X_mm 

end subroutine !dSymmGeneralEigen_lin_dep 


! tol_in should be the tolerance of the inverse condition number 
subroutine compute_neg_root_lin_dep(S, S_nrt, tol_in)
  use m_precision, only : blas_int
  implicit none
  real(8), intent(in)  :: S(:,:)
  real(8), allocatable, intent(out) :: S_nrt(:,:)
  real(8), intent(in), optional:: tol_in

  real(8):: tol
  real(8), allocatable:: E_S(:), X_S(:,:)
  ...
  do i=1, n
    if(E_S(i) .gt. 0.0d0 .and. &
         abs(E_S(i))/maxval(abs(E_S)) .gt. tol  ) then
      start=i 
      write(6,*) "Removed ", start -1, "eigenvalues"
      write(6,*) "Inverse condition number of first eigenvalue kept ", abs(E_S(i))/maxval(abs(E_S))   
      exit
    else
      write(6,*) "removed eigenvalue ", E_S(i), " with inverse condition number", abs(E_S(i))/maxval(abs(E_S))
    end if
  end do
...
end subroutine compute_neg_root_lin_dep !dSymmGeneralEigen_lin_dep 


subroutine solve_gen_eig_lin_dep(H, S_nrt, X, E)
  use m_precision, only : blas_int
  implicit none
  real(8), intent(in)  :: H(:,:), S_nrt(:,:)
  real(8), allocatable, intent(out) :: X(:,:), E(:)

  real(8), allocatable:: X_mm(:,:) , H_new(:,:)
...
end subroutine solve_gen_eig_lin_dep !dSymmGeneralEigen_lin_dep 
